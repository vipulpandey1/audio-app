...
class ReadingController extends Controller
{
    public function index(){
        $subcategory = Product::whereIn('category_id', [12,11])->get();
        return view('admin.reading.index',compact('subcategory'));
    }

    public function readingAdd(){
        $category = Category::all();
        return view('admin.reading.add',compact('category'));
    }
    ...
    public function readingEdit($id){
        $subcategory = Product::with('category','subCategory')->find($id);
        $category = Category::all();
        $subCate = SubCategory::all();
        return view('admin.reading.edit',compact('subcategory','category','subCate'));
    ...
    public function removereading($id){
        ...
        if($category->delete()){
            return redirect()->route('reading')->with("message","Product deleted successfully");
        }

    }


    public function savereading(Request $req){
        $data = $req->all();
        ...
        if($category){
            return redirect()->route('reading')->with("message","Product inserted successfully");
        }

    }


    public function singleChapterSave(Request $req){
        ...
        $query = ReadingList::updateOrCreate(["product_id"=>$req->product_id,"id"=>$req->productMediaId],$data);
        if($query){
            if(!empty($req->productMediaId)){
                return redirect()->route('reading.get-list',$req->product_id)->with("message","Product updated successfully");
            }else{
                return redirect()->route('reading.get-list',$req->product_id)->with("message","Product inserted successfully");
            }
   ...
   public function readingnewadd($id,$audio=null){
    $productId = $id;
    $proMedia = $audio;
    $media = ReadingList::where('id', $proMedia)->first();
    return view('admin.reading.addReading',compact('productId','proMedia','media'));
   }
    /**
    ...
    */
    ...
    $productId = $id;
        $accountMedias = ReadingList::where('product_id', $id)->get();
       return view('admin.reading.indexReading',compact('accountMedias','productId'));
    ...
    */
    public function deleteChapter(Request $request){
        $accountMedia = ReadingList::find($request->id);
      
        if($accountMedia->delete()){
          return back()->with("message","Chapter file deleted");
        }
    }
